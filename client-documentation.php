...
class simpleDocumentation {
	...
	private function filterData($data, $export = false){
		...
		if($export){
			...
			foreach($data as $d){
				...
				$final[] = array(
					...
					'type' => $d->type,
					'attachment_filename' => $attachment_filename['file'],
		...
		$attachment_url = $data->attachment_id ? wp_get_attachment_url( $data->attachment_id ) : null;
		$attachment_filename = $data->attachment_id ? wp_get_attachment_metadata( $data->attachment_id ) : null;
			...
			if(!empty($_POST['item']) && is_array($_POST['item'])){
				...
				if(!empty($item['type'])){
					...
					if( $item['type'] == 'note' || $item['type'] == 'video' ){
						...
						$attachment_filename = wp_get_attachment_metadata($attachment);
